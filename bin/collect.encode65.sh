...
do
	...
	x1=`cat $results/$id/scallop.$coverage/gffall.single | head -n 1 | awk '{print $10}'`
	y1=`cat $results/$id/stringtie.$coverage/gffall.single | head -n 1 | awk '{print $10}'`
	x2=`cat $results/$id/scallop.$coverage/gffall.single | head -n 1 | awk '{print $16}'`
	y2=`cat $results/$id/stringtie.$coverage/gffall.single | head -n 1 | awk '{print $16}'`
	z1=`cat $results/$id/transcomb.$coverage/gffall.single | head -n 1 | awk '{print $10}'`
	z2=`cat $results/$id/transcomb.$coverage/gffall.single | head -n 1 | awk '{print $16}'`
	echo "$id $x1 $x2 $y1 $y2 $z1 $z2"
done
